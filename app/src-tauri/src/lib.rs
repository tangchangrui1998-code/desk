...
const DEEPSEEK_KEYRING_SERVICE: &str = "com.tangchangrui.foedesk";
...
fn deepseek_keyring_entry() -> Result<keyring::Entry, String> {
    keyring::Entry::new(DEEPSEEK_KEYRING_SERVICE, DEEPSEEK_KEYRING_ACCOUNT)
...
fn validate_chat_request(model: &str, messages: &[AiChatMessage]) -> Result<(), String> {
    if model.is_empty()
        || model.len() > 100
        || !model.chars().all(|character| character.is_ascii_alphanumeric() || matches!(character, '-' | '_' | '.' | '/'))
    {
...
async fn request_deepseek(
    ...
    let body = response.text().await.map_err(|error| format!("读取 DeepSeek 响应失败：{error}"))?;
...
async fn get_deepseek_status() -> Result<AiProviderStatus, String> {
    tauri::async_runtime::spawn_blocking(|| {
        read_deepseek_api_key().map(|key| AiProviderStatus { configured: key.is_some() })
    })
    .await
...
async fn save_deepseek_api_key(api_key: String) -> Result<AiProviderStatus, String> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        deepseek_keyring_entry()?.set_password(&normalized)
...
async fn test_deepseek_connection(model: String) -> Result<AiChatResponse, String> {
    ...
    request_deepseek(
        ...
        model.trim(),
        &[
            AiChatMessage { role: "system".into(), content: "你正在进行连接测试。".into() },
            AiChatMessage { role: "user".into(), content: "只回复：连接成功".into() },
        ],
        16,
    ).await
}
...
#[tauri::command]
async fn chat_with_deepseek(model: String, messages: Vec<AiChatMessage>) -> Result<AiChatResponse, String> {
    let api_key = tauri::async_runtime::spawn_blocking(read_deepseek_api_key)
        .await
        .map_err(|error| format!("读取 DeepSeek 配置失败：{error}"))??
...
pub fn run() {
    ...
    let builder = builder.plugin(tauri_plugin_single_instance::init(|app, _args, _cwd| {
        if let Some(window) = app.get_webview_window("main") {
            ...
            let is_compact_window = window.outer_size().ok().zip(window.scale_factor().ok())
            ...
            if is_compact_window {
                ...
                if let (Some(monitor), Ok(size)) = (monitor, window.outer_size()) {
                    ...
                    let x = work_area.position.x + ((work_area.size.width as i64 - size.width as i64) / 2) as i32;
                    let y = work_area.position.y + ((work_area.size.height as i64 - size.height as i64) / 2) as i32;
